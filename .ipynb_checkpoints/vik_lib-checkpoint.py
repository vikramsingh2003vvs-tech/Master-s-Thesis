...
def fit_with_bar(func, x, y, guess, sigma=None, maxfev=1000, bounds=(-np.inf, np.inf)):
    ...
    with tqdm(total=maxfev, desc=f'Fitting  {func.__name__}',
        ...
        def _wrap(x, *args):
            ...
            return func(x, *args)
        try:
            popt, pcov = curve_fit(_wrap, x, y, p0=guess, sigma=sigma,
                                   maxfev=maxfev, bounds=bounds, ftol=1e-15, xtol=1e-15, gtol=1e-15, diff_step=1e-6)
