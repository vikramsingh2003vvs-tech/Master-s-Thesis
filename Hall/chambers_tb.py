...
@njit(parallel=True)
def _compute_sigmas_core(B_arr, G_is, G_an, cos2phi_nu, sig0_kernel,
                          dt_path_1, vx_path, vy_path, weight,
                          gt_is, gt_an, cs_1_is, cs_1_an, N):
    """Calcola sigma_xx e sigma_xy con la formula di Chambers.
    Ritorna (MR_arr, sig_xy_arr) in unità codice."""
    gamma_fs = G_is + G_an * cos2phi_nu
    sig0 = np.sum(sig0_kernel / gamma_fs)
    cs_1 = G_is * cs_1_is + G_an * cs_1_an
    gamma_total_1 = G_is * gt_is + G_an * gt_an

    MR_arr   = np.zeros(len(B_arr))
    sxy_arr  = np.zeros(len(B_arr))
    ...
    for b_idx in range(len(B_arr)):
        ...
        for i in prange(N):
            for j in range(N):
                ...
                step_int = (1.0 - np.exp(-g_safe * dt_path_1[i, j] / B)) / g_safe
                J_x_1orbit[i] += vx_path[i, j] * exp_term * step_int
                J_y_1orbit[i] += vy_path[i, j] * exp_term * step_int
        ...
        sigma_xy = 0.0
        for i in range(N):
            w = weight[i] * multiorbit
            sigma_xx += J_x_1orbit[i] * w
            sigma_xy += J_y_1orbit[i] * w

        if sigma_xx > 0:
            MR_arr[b_idx]  = sig0 / sigma_xx - 1.0
        else:
            MR_arr[b_idx]  = np.nan
        sxy_arr[b_idx] = sigma_xy

    return MR_arr, sxy_arr
...
class ChambersFitEngine:
    ...
    def __init__(self, t=435., tp=-50., tpp=38., p=0.24, nu=4,
        ...
        # ── Formula weak-field per R_H ────────────────────────────────────
        # σ_xy / B = K_MEV * ∮ (ds/vF) * vx * A(k) / γ(k)²
        # A(k) = vy * ∂²ε/∂kx∂ky − vx * ∂²ε/∂ky²
        # Per TB: ∂²ε/∂kx∂ky = −4t'·sin(kx)sin(ky)
        #         ∂²ε/∂ky²   =  2t·cos(ky) + 4t'·cos(kx)cos(ky) + 8t''·cos(2ky)
        d2_cross = -4.0 * tp  * np.sin(kx) * np.sin(ky)
        d2_yy    = (2.0*t  * np.cos(ky)
                  + 4.0*tp  * np.cos(kx) * np.cos(ky)
                  + 8.0*tpp * np.cos(2.0*ky))
        A_weak   = vy * d2_cross - vx * d2_yy
        # kernel B-indipendente: moltiplicare per K_MEV/γ² per avere σ_xy/B
        self.sxy_kernel = (ds / self.vF_fs) * vx * A_weak

        # Calibrazione: a G_an=0 R_H = V_cell/(e*(1+p))  [Drude/Luttinger]
        a_cell = 3.756e-10; c_cell = 13.2e-10
        self.RH_iso_phys = a_cell**2 * c_cell / (1.6e-19 * (1.0 + p)) * 1e9  # mm³/C
        gamma_iso = np.ones(N)  # G_is=1, G_an=0 — il fattore si cancella in R_H
        sxy_iso   = K_MEV * float(np.sum(self.sxy_kernel / gamma_iso**2))
        sxx_iso   = float(np.sum(self.sig0_kernel / gamma_iso))
        self.calib_RH = self.RH_iso_phys / (sxy_iso / sxx_iso**2)
        print(f'Calibrazione R_H (weak-field): {self.RH_iso_phys:.4f} mm³/C, '
              f'calib={self.calib_RH:.4e}')

        # Calibrazione ρ_xx(B=0): Boltzmann 2D, spin↑+↓, c_layer = c_cell
        # σ_xx^{3D} = 2 * e²/(4π²ħ * c_layer) * Σ(ds/vF)*vx²/γ
        # ρ_xx [µΩ·cm] = 10^8 * 4π²ħ*c_layer / (2*e²) / σ_xx_code
        hbar_si = 1.0546e-34; e_si = 1.6e-19
        self.calib_rho = 1e8 * 4.0 * np.pi**2 * hbar_si * c_cell / (2.0 * e_si**2)
        print(f'Calibrazione ρ_xx(B=0): calib_rho = {self.calib_rho:.1f} µΩ·cm/code')
    ...
    def compute_rho_xx(self, G_is, G_an):
        """ρ_xx a B=0 in µΩ·cm (calibrazione first-principles, 2 spin).

        ρ_xx = calib_rho / Σ_i (ds_i/vF_i)*vx_i²/γ_i
        """
        gamma_fs = G_is + G_an * self.cos2phi_nu
        sxx_code = float(np.sum(self.sig0_kernel / gamma_fs))
        return self.calib_rho / sxx_code

    def compute_RH(self, G_is, G_an):
        """R_H nel limite di campo debole [mm³/C], B-indipendente.

        Formula: R_H = K_MEV * ∮(ds/vF)·vx·A(k)/γ² / [∮(ds/vF)·vx²/γ]²
        con A(k) = vy·∂²ε/∂kx∂ky − vx·∂²ε/∂ky²  (derivata della velocità lungo l'orbita).
        """
        gamma_fs = G_is + G_an * self.cos2phi_nu
        sxy_weak = K_MEV * float(np.sum(self.sxy_kernel / gamma_fs**2))
        sxx0     = float(np.sum(self.sig0_kernel / gamma_fs))
        return self.calib_RH * sxy_weak / sxx0**2